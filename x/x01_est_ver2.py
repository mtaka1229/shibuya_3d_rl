...
param = 10
####### 観測方程式 ###### 
def ddr(rssi, dens, param):
    return (10**((-rssi-50)/dens/param)+10) # x[0]は非零．RSSI0=-50は根拠なし
...
def measuring_model1(x): 
    ...
    for file_name in file_list: 
        ...
        ### これは，以下の内容を関数として外で定義して，各ファイルに対してapply関数か何かで一気に処理させる方が早いのか？？
                ...
                if timestep_delta == 1: # 欠損ない場合，過去内容を踏襲
                    for j in range(len(df_now)):
                        ...
                        for k in range(len(df_link_odds)):
                            ...
                            # ここでdf_nodeを呼び出すのがいい，はず，すぐ書き換えられそう
                            # リンクとの交差判定，一個も引っ掛からんかったものは最寄りのリンクに割り当てる
                            # 
...
def loglikelihood(x):
    ...
    for d in range(D):
        ...
        for i in range(len(df_list2)): #iは個人
            ...
            #timestep2の場合，1がうまく取れてないと0になって後ろが死ぬ．なので，timestep3からやってみる．t:3~len(q)
                # 時刻tにおいてq != 0のlinkidを取得→measured_link
                measured_links = np.where(q[t] != 0)[0] # list形式．indexのはず
                measured_links = measured_links + 1 # linkidになってるはず
                lt = 0
                #print(f'観測確率非零のlinkのlinkid{measured_links}')
                # timestep tにおける各観測リンクに対しての操作
                ...
                #print(f'時刻{t}で尤度ltは{lt}') # 時刻1でltが0になっている→残り全部0になってしまう，，，，
                # ltが0ならliが0になってlliが0になってしまう．
                li = li * lt
                #print(f'時刻{t}で尤度liは{li}')
                        
            # これで個人iの尤度liがでた．これを対数化
            #print(li) # liが0になっているのでlogに入れられない
            
            # lliが0ならcontinue→次のiに
            ...
            if li == 0:
                ...
                lli = math.log(li)
                #print(f'liの値は非零で{li}で，logとると{lli}')
                Ld += math.log(li) 
                #print(f'今LLは{LL}です')
                count += 1

        LL += Ld
        print(f"link{d}の操作終わり，計算したuserは{i}人で，今尤度は{LL}です")
...
while dL >= 5:
    ...
    z = z0 # 価値関数固定
    # 構造推定part1：対数尤度関数最大化
    # res = minimize(loglikelihood, x, bounds = bounds, method='BFGS', tol = 5) #options={"maxiter":10,"return_all":True}) 
