...
# あらかじめevensとodds組み合わせておく．ここには仮想リンクと吸収リンクを入れている
...
# 00_prep_nw.pyでnode_preの方に保存したので．
...
# 17:00-17:15のデータ
search_folder = "/Users/takahiromatsunaga/res2023/bledata/ble_timegroup/20221218_17/quater_address50"
file_list = list(os.listdir(search_folder))

####### リンク接続行列（無向） ####### 
def I_nod(link_data): # odds or evensを読み込む想定
    n = len(link_data)
    ...
    for i in range(n):
        ...
        for j in range(n):
            if ((link_data.loc[j, 'O'] == O) or (link_data.loc[j, 'O'] == D)) or (link_data.loc[j, 'D'] == O) or (link_data.loc[j, 'D'] == D): 
                # 当該Onode or Dnodeをo or dとするリンクを全部1にしている．つまり無向グラフと同じなので，odds読み込んでもちゃんとevensの分も行ってる！！！！！
                I[i, j] = 1
    return(I)
...
###### 観測モデル内で使うためのデータ準備 ####### # 仮想リンクをlinkdataに入れた後の方がいいかもしれない，，，，
gate_link = [59, 61, # 1F南
            43, 47, 53, # 1Fハチ公
            35, 37] # 3F中央
...
####### 観測モデルその1 ###### # 逆方向リンクや仮想リンクはまだやらない
def measuring_model1(x): # count = 0
    
    # 最終アウトプットの初期設定（NaN）吸収以外の全リンク入れる．つまりlinkidは1~117まで（pre）
    ...
    for file_name in file_list: 
        if file_name.endswith('.csv'): # DS_Storeファイルの読み込み回避
            ...
            prev_ts = base_ts # 初期設定．i=1の時だけこれが使われる
            ...
            for i in range(timestep_count):
                ...
                # 1timestep内での観測尤度を入れるための配列．各linkの観測確率を入れるので吸収以外のリンク数117．
                # 後でres_unionに結合．1timestepでリセットされ，非連続の場合は使われない
                res_t = np.zeros(117) # res_t = np.zeros(len(df_link_odds)) 
                ...
                if timestep_delta != 1: # 欠損を補完するコード
                    ...
                    for candidate in prev_link:
                        ...
                        # candidates = connectlink_nod(candidate) # 無向グラフのあるリンクに対して接続リンクの集合リストを与える関数
                        # candidateはlinkid（1~117）だがIndは奇数列しかないので50*50．candidiateのindexはcandidate//2
                        cand_column = Ind[:, candidate//2] # candidate-1がcandidateのindex
                        cands_index = np.where(cand_column == 1)[0] # candidateに接続するリンクcandsのindexの集合を得る．接続行列Idを参照
                ...
                if timestep_delta == 1: # 欠損ない場合，過去内容を踏襲
                    for j in range(len(df_now)):
                        ...
                        # 各データiの検出bleの座標を取得 # IDに対応するbleデータをdf_bleからとって，その行の'x', 'y', 'floor'を読む
                        # x_ap = df_now.loc[j, 'x'] y_ap = df_now.loc[j, 'y'] z_ap = df_now.loc[j, 'floor']*1000 
                        ble_index_list = df_ble[df_ble['ID']==ID].index.tolist() # index()の()が要らなかったらしい．中身の要素は一つだけのはず．index()はindexオブジェクトを返す！
                        ble_index = ble_index_list[0]

                        x_ap = df_ble.loc[ble_index, 'x']
                        y_ap = df_ble.loc[ble_index, 'y']
                        z_ap = df_ble.loc[ble_index, 'floor']*1000 # zの値をめちゃ大きくすることで，階を挟んだ電波捕捉を捨象（階の厳密な高さが不明なのと，床板・天井版による電波遮蔽を考慮するため）．階が同じなら0になるので変な影響はない

                        for k in range(len(df_link_odds)):
                            linkid = df_link_odds.loc[k, 'linkid']
                            #index = linkid // 2 # judgeでのindex．linkidは奇数なので列はlinkid//2+1列目．indexは列数-1なので結局linkid // 2 ## judgeでカウントする際に必要になる
                            index = linkid - 1

                            x_o = df_link_odds.loc[k, 'x_o']
                            y_o = df_link_odds.loc[k, 'y_o']
                            z_o = df_link_odds.loc[k, 'z_o']*1000

                            x_d = df_link_odds.loc[k, 'x_d']
                            y_d = df_link_odds.loc[k, 'y_d']
                            z_d = df_link_odds.loc[k, 'z_d']*1000

                            x_mid = df_link_odds.loc[k, 'x_mid']
                            y_mid = df_link_odds.loc[k, 'y_mid']
                            z_mid = df_link_odds.loc[k, 'z_mid']*1000
                        
                            x_qu = x_o/4 + x_d/4*3 # 1/4点
                            y_qu = y_o/4 + y_d/4*3
                            z_qu = (z_o/4 + z_d/4*3) * 1000

                            x_qua = x_o/4*3 + x_d/4 # 3/4点
                            y_qua = y_o/4*3 + y_d/4
                            ...
                            if d < ddr(rssi, dens, x):
                                judge[index] = 1 # DDR内判定されたリンクのところが1になる
            ...
            if (o_link in staire_link) or (d_link in staire_link): # o_linkあるいはd_linkが階段リンクだった場合，ループを飛ばす
                continue
            if d_link in non_gate_link: # dがnon_gate_linkの場合落とす
                continue
...
def measuring_model2(x):
    ...
    for i in range(len(df_list)):
        ...
        # あとはお尻から次のtimestep（timestepは1からじゃないよー）での観測確率が最大のリンクのo_nodeに近い方をとる．# こうすることで回遊とか彷徨い行動をカットしないで済む（それだけのためである）
...
###### 経路選択モデル ####### # 一回観測モデルの出力をcsvにして読み込む形で選択モデルを回す

###### 初期設定 ######
ddata = [118, 119] # 吸収リンクのlinkid
# 118のやつがないかもしれない，，
...
def Mset(x): 
    ...
    return cost_numpy # 出力はdataframe → numpyのつもりだった
...
def loglikelihood(x):
    ...
    for d in range(D):
        ...
        if len(df_list) == 1:
            ...
            for d in range(D):
                print(d)
                df = df_list[d] # absroptionがdのデータの集合（順番保たれてるよね？z→個人で分けた時にtimestepの昇順に変換すれば良い話
                ...
                for i in range(len(df_list2)):
                    ...
                    li = 1 # 個人ごとの初期の尤度
                    for t in range(1, len(q)): # timestep1の尤度とかはない，timestep2以降なのでt:1~len(q)
                        # 時刻tにおいてq != 0のlinkidを取得→measured_link
                        measured_links = np.where(q[t] != 0)[0] # list形式
                        measured_links = measured_links + 1
                        lt = 0
                        for measured_link in measured_links:
                            l_for_each = 0
                            measured_column = Id[:, measured_link-1]
        ...
        print(f"link{d}の操作終わり，尤度は{LL}です")
...
x_init = x0
bounds = [(0, None), (None, None), (None, None), (None, None), (None, None), (None, None), (None, None), (None, None), (0, 1)]
...
while dL >= 0.01:
    ...
    z = z0 # 価値関数固定
    # 構造推定part1-----対数尤度関数最大化
    res = minimize(loglikelihood, x, bounds = bounds, method='L-BFGS-B', options={"maxiter":100,"return_all":True}) 
    # 条件は後で指定，beta(x[8])とx[0]の範囲を指定する必要がある！x[8]は0-1，x[0]>0
